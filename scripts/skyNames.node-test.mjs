...
test('constellation names: IAU Latin names with J2000 label points', () => {
  ...
  assert.ok(json.data.some((row) => row[1] === 'Canis Major'));
  assert.ok(json.data.some((row) => row[1] === 'Corona Australis'));
  for (const [id, name, ra, dec, rank] of json.data) {
    assert.match(id, /^[A-Z][A-Za-z0-9]{1,4}$/, id);
    // IAU names, ASCII spaces only -- one space between words, no other
    // whitespace (some Latin variants upstream use U+2005 between words).
    assert.match(name, /^[A-Za-zö]+( [A-Za-zö]+)*$/, name);
