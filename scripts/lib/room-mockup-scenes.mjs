 ...
 *
 * Both scenes crop public/images/backdrops/our-story-light.jpg (2560 x 1714),
 ...
 * MOCKUP_OUTPUT.
 *
 * Scale honesty: the detail crop contains no scale reference, so the print
 * may fill the frame like any close-up product shot. The context crop keeps
 * the floor lamp (IKEA-style shade, ~31 cm across ~265 source px, i.e.
 * ~8.5 px/cm) and sizes the 20.3 cm-wide print to ~173 source px so it
 * appears at its true 8 x 10 in size next to the lamp.
 */
...
export function mockupRelativePath(catalogImagePath, scene) {
  ...
  const directory = relative.includes('/')
    ...
    : '';
  const file = mockupFileName(relative.slice(relative.lastIndexOf('/') + 1), scene);
