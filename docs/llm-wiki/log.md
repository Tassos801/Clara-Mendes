...
## 2026-09-24 - Your Sky: label spacing and two-line titles

Constellation labels are spaced with EB Garamond Regular's real advance widths
(`app/lib/sky/labelMetrics.ts`, generated by
`node scripts/build-sky-label-metrics.mjs`) plus the tracking and 0.25 em of
air each side, instead of a 0.66 em estimate that let pairs such as CORVUS /
CRATER print touching. Two-line titles are capped per layout
(`titleTwoLineMaxSize` in `app/lib/sky/layouts.ts`, passed to `fitTitle`) so
the top line clears the S cardinal and the bottom line clears the subtitle;
one-line titles and First Light are unchanged. Tests: `scripts/skyLabels.node-test.mjs`
